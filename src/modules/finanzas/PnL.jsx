...
function Row({ label, value, indent=0, bold=false, color, borderTop=false, sub }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', padding:'5px 0', paddingLeft: indent*16, borderTop: borderTop?`1.5px solid ${T.border}`:'none' }}>
      <span style={{ fontSize:'.86rem', color: color||T.dark, fontWeight: bold?700:400 }}>
        {label}{sub && <span style={{ fontSize:'.72rem', color:T.mid, fontWeight:400, marginLeft:6 }}>{sub}</span>}
      </span>
...
export default function PnL({ pnl, movimientos }) {
  const { ventasWalmart, ingresosBanco, felEmitidas, notasCredito, ivaRetenido,
  ...
  return (
    <div style={{ maxWidth:560 }}>
      ...
      <Section title="Ingresos">
        <Row label="Ventas Walmart (salidas bodega)" value={ventasWalmart} indent={1} color="#15803d" />
        {ingresosBanco  > 0 && <Row label="Otros cobros banco" value={ingresosBanco} indent={1} />}
        {felEmitidas    > 0 && <Row label="Facturas FEL emitidas" value={felEmitidas} indent={1} />}
      ...
      <Section title="Costos de producto (banco)">
      ...
      <Section title="Gastos fijos (banco)">
      ...
      <Section title="Gastos variables (banco)">
      ...
      <Section title="Punto de equilibrio">
        <Row label="Total gastos fijos" value={gastosFijos}  indent={1} />
        ...
        <div style={{ fontSize:'.82rem', fontWeight:600, color: pctEq>=100?'#15803d':pctEq>=80?T.warn:T.danger }}>
          {pctEq>=100 ? `✅ Punto de equilibrio superado (${fmtP(pctEq)})`
            : `Faltan ${fmtQ(Math.max(0,puntoEquilibrio-ingresoNeto))} para cubrir fijos (${fmtP(pctEq)})`}
        </div>
      </Section>
    </div>
  );
